...
2026-07-09 最新进展：C2 的 RoPE `position_offset`、terminal tail trim、`token_counts` 默认 reset、三档 `kv_terminal_drop_mode`、bounded token-history re-prefill 诊断均已落地并推送；但 C2 门禁复测仍未通过。tail 长度 sweep 的最佳 3 样本 smoke 为 30.19% CER，embedding replay re-prefill 为 50.69% CER，三档 drop-mode smoke 显示 `eos_only=31.62%`、`silence=31.05%`、`pad_phase=58.48%`，bounded token-history re-prefill 为 55.22% CER。结论是：`pad_phase` 明确过裁并显著恶化复读，但补回上一段 text token 的受限 re-prefill 仍会复读，说明当前基座/当前 TRT profile 下的 C2 还不能作为表 2 正式行；完整 C4-style re-prefill 还受 `max_input_len=128` 约束，需要重编 Phase B profile 或走 PyTorch 诊断后再决定 C4 训练。
...
| 4 | 仅 KV/token 尾 C2 prototype | P0 位置修复 + terminal drop + bounded token-history 诊断已落地，门禁仍失败 | 修复前 27.01% +/- 0.55%；最新 smoke 最好 30.19%-31.05%；bounded token-history 55.22% | RoPE/早停/过裁都已量化，但文本完整性仍失败；下一步解决完整 re-prefill 的 `max_input_len=128` 限制。 |
...
| 问题 | 现象 | 原因判断 | 处理策略 |
|---|---|---|---|
| C2/C3 CER 变差 | C2 到 27.01%，C1+C2+C3 到 28.24%；修复后 3 样本 smoke 仍在 30%-55% CER | 已定位四层问题：RoPE position 错位会破坏 compact KV；`pad_phase` 会一次丢掉 73%-95% 尾部帧造成严重复读；`eos_only/silence` 仍约 31% CER；bounded token-history 受 `max_input_len=128` 只能保留 37-42 帧 code tail，仍复读 | `position_offset`、terminal drop mode、裁剪比例 metrics、token-history carry 均已落地；下一步不是继续调 tail，而是验证完整 token-history re-prefill 是否需要重编 TRT profile/走 PyTorch。 |
...
| C2 token-history 诊断 | 新增 `kv_reprefill_token_history=true`：保存每步 `full_codec`，carry 上一段 text token ids + codes，按 TRT profile 构造 `prefix + history text + history codes tail` bounded re-prefill，当前段仍走 request/trailing streaming | 机制已通过单测并提交；3 样本 smoke 为 55.22% CER，证明受限 token-history 不足以通过 C2 门禁。 |
...
| P0 | 用 P0 C2 修复重跑 `kv_tail_only` 和 `tail_kv_pause_recovery` | 已完成 3 样本门禁 smoke：trim-tail 默认 384 为 65.57%/63.51% CER；tail sweep 最佳 `kv64` 为 30.19% CER；embedding replay re-prefill 为 50.69% CER；drop-mode 最佳 `silence` 为 31.05% CER；bounded token-history 为 55.22% CER。均未达标。 |
| P0 | 如果修复后 C2 仍不达标，继续补 sink/window/token-history re-prefill 或 position-offset KV 细节 | 已新增 `kv_reprefill_history=true`、`kv_terminal_drop_mode`、`kv_reprefill_token_history=true` 并提交 `0179be1`、`b0512b2`、`102d688`、`5e02b87`。结论：over-trim 是 `pad_phase` 恶化主因，但不是 C2 不达标的唯一主因；下一步要先突破 `max_input_len=128` 的完整 re-prefill 限制，再决定是否具备启动 C4 的前提。 |
...
7. 因此当前实现还没有达到计划里的“sink + 近 W 秒文本+语音 token 序列重新 prefill”。完整 C4-style re-prefill 需要更长 prefill profile 或 PyTorch 诊断，否则无法判断“完整 text+codes 排布”本身是否可行。

### 8.4 当前卡点和决策

当前卡点是 C2 的语义 continuation 形态仍不正确，而且完整 token-history re-prefill 被现有 TRT Phase B `max_input_len=128` 卡住。旧 KV carry、position-offset KV、terminal trim、tail sweep、embedding replay、drop-mode sweep 和 bounded token-history 都不能让 CER 回到基线。下一步有两个可解释路径：重编 Phase B TRT profile 支持更长 `input_embeds`，或用 PyTorch/离线诊断先验证 `prefix/sink + 历史 text + 历史 codes 尾窗 + 当前 text` 的完整排布；否则 C2 应在表 2 中标注为未通过，不应启动 C4 正式训练。
