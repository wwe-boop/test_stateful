...
class TestPrefillBoundary:
    ...
    def test_icl_prefix_cache_miss_then_hit_prefills_suffix_tokens(self, model_config):
        loop = _ImmediateLoop()
        executor = _StubExecutorForPrefill(model_config)
        hidden = model_config.hidden_size

        cacheable = torch.randn(1, 3, hidden)
        suffix = torch.randn(1, 2, hidden)
        trailing = [torch.full((1, 1, hidden), 2.0)]
        plan = PrefillPlan(
            prefill_embeds=torch.cat([cacheable, suffix], dim=1),
            trailing=trailing,
            prefix_cache_key="icl-key",
            cacheable_prefix_embeds=cacheable,
            request_prefill_embeds=suffix,
        )
        builder = _StubPrefillBuilder(plan=plan, hidden_size=hidden)
        ...
        first_results = queue.Queue()
        cfg = SessionConfig(
            task_type="voice_clone",
            x_vector_only=False,
            ref_source="registry",
            ref_id="Vivian",
            ref_audio_sha256="abcdef1234567890",
            ref_text_hash="123456abcdef7890",
        ...
        assert engine_loop._try_prefill_one() is True
        assert len(executor.prefill_prefix_only_inputs) == 1
        assert len(executor.prefill_from_prefix_inputs) == 1
        torch.testing.assert_close(executor.prefill_prefix_only_inputs[0], cacheable)
        torch.testing.assert_close(executor.prefill_from_prefix_inputs[0], suffix)
        first_prefill_done = first_results.get_nowait()
        assert first_prefill_done.type == ResultType.PREFILL_DONE
        assert first_prefill_done.metrics["icl_cache_miss"] == "true"
        assert first_prefill_done.metrics["ref_source"] == "registry"
        assert first_prefill_done.metrics["ref_audio_sha256"] == "abcdef123456"

        second_results = queue.Queue()
        cfg2 = SessionConfig(
            task_type="voice_clone",
            x_vector_only=False,
            ref_source="registry",
        ...
        assert engine_loop._try_prefill_one() is True
        assert len(executor.prefill_prefix_only_inputs) == 1
        assert len(executor.prefill_from_prefix_inputs) == 2
        torch.testing.assert_close(executor.prefill_from_prefix_inputs[1], suffix)
        second_prefill_done = second_results.get_nowait()
        assert second_prefill_done.type == ResultType.PREFILL_DONE
        assert second_prefill_done.metrics["icl_cache_hit"] == "true"

    def test_icl_prefix_too_long_falls_back_to_full_prefill(self, model_config):
        loop = _ImmediateLoop()
        executor = _StubExecutorForPrefill(model_config)
        hidden = model_config.hidden_size

        cacheable = torch.randn(1, 4, hidden)
        suffix = torch.randn(1, 2, hidden)
        plan = PrefillPlan(
            prefill_embeds=torch.cat([cacheable, suffix], dim=1),
            trailing=[],
        ...
        torch.testing.assert_close(executor.prefill_inputs[0], plan.prefill_embeds)
        prefill_done = result_queue.get_nowait()
        assert prefill_done.metrics["icl_cache_miss"] == "skipped_prefix_too_long"
