...
against FP32 PyTorch reference (e2e_trt_ref.npz from tests/tools/verify_e2e_trt_ref.py).

Usage (inside NGC container or host with TensorRT):
  python3 tests/tools/verify_e2e_trt.py --model-dir /path/to/exported/variant [--ref-file /path/to/e2e_trt_ref.npz]
"""
...
def main():
    ...
    if not os.path.exists(ref_file):
        logger.error(
            f"Reference file not found: {ref_file}. Run tests/tools/verify_e2e_trt_ref.py on host first."
        )
        sys.exit(1)
